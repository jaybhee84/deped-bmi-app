...
export default function BatchEntry({ students, setStudents, currentUser }) {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        const importedRows = results.data.map((row) => {
          ...
          let cleanedName = rawName.replace(/^[\d\s.\-]+/, "").toUpperCase();
  ...
  function saveAll() {
    ...
    // --- RECYCLING REGISTRY NUMBERS ---
    // Extract set of all currently assigned registry numbers from active students
    const existingRegistryNumbers = new Set(
      students.map((s) => s.registryNo).filter(Boolean),
    );

    // Iteratively generate registry candidates and grab the first available ones (filling gaps left by deleted learners)
    const assignableRegistryNumbers = [];
    let count = 1;
    while (assignableRegistryNumbers.length < needsRegistry.length) {
      const candidateNums = generateRegistryNumbers(
        sy,
        gradeLevel,
        schoolName,
        1,
      ...
      const candidate = candidateNums[0];

      if (!existingRegistryNumbers.has(candidate)) {
        assignableRegistryNumbers.push(candidate);
      }
      count++;
    }
    ...
    setStudents((prev) => {
      ...
      valid.forEach((row) => {
        ...
        const formattedName = row.name.replace(/^[\d\s.\-]+/, "").toUpperCase();
        ...
        if (existing) {
          ...
          const registryNo = row.lrn.trim()
            ? undefined
            : assignableRegistryNumbers[regIdx++];
